...
def getTerraformOutput(projectPath, output):
    ...
    return std_out.rstrip().decode("utf-8") 
...
def main(argv):
    ...
    # Retrieve autoscaling group defined in Terraform name
    ...
    # Retrieve autoscaling groups' current live information
    info = getAutoscalingInfo(agBlue, agGreen)

    # Get the which autoscaling group is active --- this is the Blue
    active = getActive(info)

    # Bring up the non-active, new autoscaling group with the new AMI --- this is the Green
    desiredInstanceCount = newAutoscaling(info, active, ami, command, projectPath, environment, inactiveDesired)

    # Parse info to get info on all ELBs/ALBs
    elbs = getLoadbalancers(info, 'elb')
    albs = getLoadbalancers(info, 'alb')

    # Retrieve new live autoscaling groups information
    #(we need to do this again because the launchconfig has changed and we need this in a later phase)
    info = getAutoscalingInfo(agBlue, agGreen)

    #have to insert code to kill blue

    if 'apply' in command:
        print ('Waiting for 30 seconds to get autoscaling status')
        time.sleep(30)
        timeout = 30
        #loop till any instance health (new or old) is unhealthy
