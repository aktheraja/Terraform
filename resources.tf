...
//routing table for pubic subnet 1
resource "aws_route_table" "route-public_1" {
	vpc_id = "${aws_vpc.vpc_environment.id}"
	...
	tags {
		Name = "Public Subnet 1 Routing Table"
	}
}

//routing table for pubic subnet 2
resource "aws_route_table" "route-public_2" {
	vpc_id = "${aws_vpc.vpc_environment.id}"
	...
	tags {
		Name = "Public Subnet 2 Routing Table"
	}
}
...
resource "aws_lb" "alb" {
	...
	security_groups = ["${aws_security_group.private_subnetsecurity.id}", "${aws_security_group.public_subnetsecurity.id}"]
	internal        = false
	idle_timeout    = 60
	tags {
		Name    = "alb"
